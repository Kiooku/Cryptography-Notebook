...
def hmac(message: str, key: str, hash_func) -> str:
    """HMAC algorithm
    ...
    encoded_key: bytes = key.encode()

    if len(encoded_key) > block_size:
        encoded_key = hash_func(encoded_key).digest()
    else:
        encoded_key = encoded_key + b"\x00" * (block_size - len(encoded_key))

    k_ipad: bytes = bytes(k ^ p for k, p in zip(encoded_key, ipad))
    k_opad: bytes = bytes(k ^ p for k, p in zip(encoded_key, opad))

    return hash_func(k_opad + hash_func(k_ipad + message.encode()).digest()).hexdigest()

if __name__ == "__main__":
    # All examples are from https://www.ietf.org/rfc/rfc5869.txt and https://en.wikipedia.org/wiki/HMAC
    k1: str = "key"

...
pub fn create_prekey_bundle(ik: &IdentityKey, spk: &SignedPrekey, opk_bundle: &Vec<OneTimePrekey>) -> (PublicKey, PublicKey, Vec<PublicKey>, Signature, VerifyingKey) {
...
pub fn get_opk_used(opkb_used: PublicKey, opkb_bundle: Vec<OneTimePrekey>) -> Option<OneTimePrekey> {
    for key in opkb_bundle {
        if key.public_key == opkb_used {
            return Some(key)
        }
    }
    None
